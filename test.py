...
if selectedPage == 'Home':
    ...
    if st.session_state.data is not None:
        with st.spinner("Setting up..."):
            h1, h2 = st.columns(2)
            h1.header('Welcome to your data,')
            # Display Current Date
            currentDate = datetime.datetime.now().strftime("%B %d, %Y")  # Format: Month day, Year
            h2.subheader(currentDate)

            # Configure columns
            col1, col2 = st.columns(2)
            col1.header(f'Observation Count: {st.session_state.data.shape[0]}')
    ...
    if st.session_state.data is not None:
        ...
        with st.spinner("Plotting..."):
            with st.expander('Customize plot'):
                selectedColor = st.selectbox('Select a color ', mlc.TABLEAU_COLORS.keys())
            fig, ax = plt.subplots(figsize=(17, 7))
            sns.histplot(x=xAxisColumn, data=st.session_state.cleanData, kde=kdeCheckbox, color=selectedColor)
            plt.xlabel(xAxisColumn)
            st.pyplot(plt)
            currentGraph = plt
    ...
    if st.session_state.data is not None:
        with st.spinner("Setting up..."):
            ...
            with st.expander('Outlier Detection'):
                ...
                st.pyplot(plt)
                outliers = tools.detectOutliers(st.session_state.data[selectedNumCols])
            ...
            with st.expander('Distribution Analysis'):
                ...
                # Calculate and display relevant summary stats
                st.subheader('Summary Stats')
                meanVal = np.mean(st.session_state.data[selectedDistCol])
                medianVal = np.median(st.session_state.data[selectedDistCol])
                stdDev = np.std(st.session_state.data[selectedDistCol])
    ...
    if st.session_state.data is not None:
        with st.spinner("Setting up..."):
            ...
            linReg, logReg = st.tabs(["Linear", "Logistic"])

            with linReg:
                # Select the target variable
                targetVar = st.selectbox('Select Target Variable:', st.session_state.cleanData.columns, key='linear')
                ...
                if predictorVars:
                    ...
                    st.error(
                        "You must select at least one predictor variable")
            with logReg:
                # Select the target variable
                targetVar = st.selectbox('Select Target Variable:', st.session_state.cleanData.columns, key='logistic1')
                # Predictor variables
                predictorVars = st.multiselect('Select Predictor Variables:', st.session_state.cleanData.columns,
                                               default=st.session_state.cleanData.columns[0], key='logistic2')
                # Logistic regression
                X = st.session_state.cleanData[predictorVars]
                y = st.session_state.cleanData[targetVar]
                ...
                if all(0 <= value <= 1 for value in y):
                    ...
                    st.error("The data is not suitable for logistic regression. Please check your dependent variable (endog). It should represent binary outcomes or probabilities between 0 and 1.")
    else:
        tools.uploadFile()
